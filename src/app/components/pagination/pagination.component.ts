...
export class PaginationComponent implements OnInit {
  ...
  showView(options: number): void {
    alert("index is: " + options);
  }
  ...
  findLastPage(): void {
    this.lastPage = Math.ceil((this.comments.length) / this.pageLimit);
  }

  changePageButtons(): void {
    this.buttons = [];
    let buttons: number[] = [];
    let startButton = this.page - 2 > 0 ? this.page - 2 : 1;
    let endButton = startButton + this.noOfButtons >= this.lastPage ? this.lastPage + 1 : startButton + this.noOfButtons;
    for (let i = startButton; i < endButton; i++) buttons.push(i);
    this.buttons = buttons;
  }

  setPage(page: number): void {
    this.page = page;
    let startVal = (this.page - 1) * this.pageLimit;
    let endVal = startVal + this.pageLimit;
    this.filteredComments = this.comments.slice(startVal, endVal);
